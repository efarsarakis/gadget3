...
#define SKIP2 {my_fread(&dummy2, sizeof(int), 1, fd);}
      ...
      for(num = 0, skip = 0; num < numfiles; num++)
	{
	  ...
	  if(num == 0)
		...
		Nglass += header1.npartTotal[k];
	      ...
	      printf("\nNglass= %d\n\n", Nglass);
	      pos = (MyReal *) malloc(sizeof(MyReal) * Nglass * 3);

	      if(!(pos))
		{
		  printf("failed to allocate %g Mbyte on Task %d for glass file\n",
			 sizeof(MyReal) * Nglass * 3.0 / (1024.0 * 1024.0), ThisTask);
		  FatalError(112);
		}
	    }

	  SKIP;
	  my_fread(&pos[3 * skip], sizeof(MyReal), 3 * nlocal, fd);
	  SKIP2;

	  if(dummy != sizeof(MyReal) * 3 * nlocal || dummy2 != sizeof(MyReal) * 3 * nlocal)
	    {
  ...
  MPI_Bcast(&pos[0], sizeof(MyReal) * Nglass * 3, MPI_BYTE, 0, MPI_COMM_WORLD);
  ...
  for(i = 0; i < GlassTileFac; i++)
      ...
      for(type = 0, n = 0; type < 6; type++)
	{
      ...
      if(ThisTask == 0)
	{
	  printf("i=%d | GlassTileFac=%d  (%g sec)\n", i, GlassTileFac, t1 - t0);
	  fflush(stdout);
	}
    }
  ...
  for(i = 0; i < GlassTileFac; i++)
      ...
      for(type = 0, n = 0; type < 6; type++)
	{
	  for(m = 0; m < header1.npartTotal[type]; m++, n++)
		...
		IDStart += GlassTileFac * GlassTileFac;
	    }
	}
