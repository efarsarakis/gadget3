...
#ifdef LT_METAL_COOLING
      LambdaNet = CoolingRateFromU(u, rho * m / m_old, ne_guess, Z, temp);
#else
...
#ifdef CS_WSS
  return cs_CoolingRate_WSS(log10(temp), rho, ne_guess);
#else /* No WSS */
...
/* Note: WSS already takes heating into account self-consistently. */

double cs_CoolingRate_WSS(double logT, double rho, double *nelec)
{
  ...
  /* METALS: the second input parameter is the metallicity [Fe/H] */

  Lambda = cs_get_Lambda_WSS(logT, FeHgas, nHcgs);

#ifdef CS_WSS_ADDCOMPTON	/* Add Compton cooling manually (tabulated CC should not be used at high redshift) */
