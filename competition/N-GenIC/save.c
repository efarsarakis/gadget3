...
void write_particle_data(void)
{
  ...
  for(groupTask = 0; groupTask < nprocgroup; groupTask++)
	...
	save_local_data();
      ...
      if(ThisTask == 0)
	{
	  printf("writing of group %d out of %d took %g sec\n", groupTask, nprocgroup, t1-t0);
	  fflush(stdout);
	}
    }
...
#define BUFFER 10
...
#ifdef NEUTRINOS
  header.mass[1] = (Omega  - OmegaDM_2ndSpecies) * 3 * Hubble * Hubble / (8 * PI * G) * pow(Box, 3) / TotNumPart;
#endif
...
#ifdef NEUTRINOS
  header.mass[0] = (OmegaBaryon) * 3 * Hubble * Hubble / (8 * PI * G) * pow(Box, 3) / TotNumPart;
  header.mass[1] = (Omega - OmegaDM_2ndSpecies - OmegaBaryon) * 3 * Hubble * Hubble / (8 * PI * G) * pow(Box, 3) / TotNumPart;
#endif
  ...
  blockmaxlen = bytes / (3 * sizeof(MyReal));
  ...
  /* write coordinates */
  dummy = sizeof(MyReal) * 3 * NumPart;
#ifdef  PRODUCEGAS
  dummy *= 2;
#endif
#ifdef NEUTRINO_PAIRS
  dummy =
    sizeof(MyReal) * 3 * (header.npart[0] + header.npart[1] + header.npart[2] + header.npart[3] +
  ...
  for(i = 0, pc = 0; i < NumPart; i++)
      ...
      if(P[i].Type == 2)
	{
      ...
      if(pc >= (blockmaxlen - 1))
	{
	  my_fwrite(block, sizeof(MyReal), 3 * pc, fd);
	  pc = 0;
	}
    }
  if(pc > 0)
  ...
  for(i = 0, pc = 0; i < NumPart; i++)
    {
      ...
      if(pc == blockmaxlen)
	{
	  my_fwrite(block, sizeof(MyReal), pc, fd);
	  pc = 0;
	}
    }
  if(pc > 0)
  ...
  if(header.npart[0])
    {
      dummy = sizeof(MyReal) * header.npart[0];
      my_fwrite(&dummy, sizeof(dummy), 1, fd);
