...
void read_power_table(void)
{
  ...
  do
      ...
      if(fscanf(fd, " %lg %lg %lg %lg %lg %lg %lg", &k, &T_cdm, &T_b, &dummy, &dummy, &T_nu, &T_tot) == 7)
	{
	  ...
	  lD = pow(k, PrimordialIndex) * k * k * k * T_cdm * T_cdm;
	  lDNU = pow(k, PrimordialIndex) * k * k * k * T_nu * T_nu;
	
	  PowerTable[NPowerTable].logD = log10(lD);
	  PowerTable[NPowerTable].logD2nd = log10(lDNU);
	  NPowerTable++;
	}
      else
	break;
...
void add_WDM_thermal_speeds(MyReal *vel)
{
...
void add_NU_thermal_speeds(MyReal *vel)
{
  double v, phi, theta, vx, vy, vz;
