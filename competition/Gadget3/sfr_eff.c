...
void cooling_only(void)		/* normal cooling routine when star formation is disabled */
{
  ...
  for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
...
#endif
	{
	  ...
	  unew = Um_DoCooling(uold, SphP[i].d.Density * a3inv, dtime, &ne, i, 1);	/* flag=1: cooling */
#else
...
/* cooling routine when star formation is enabled */
{
  ...
  for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
      ...
      if(P[i].Type == 0)
	{
	  ...
	  if(SphP[i].DelayTime > 0)
#ifdef QUICK_LYALPHA
	    if(SphP[i].d.Density < All.WindFreeTravelDensFac * All.OverDensThresh)
#else
...
#ifdef UM_CHEMISTRY
	      unew = Um_DoCooling(uold, SphP[i].d.Density * a3inv, dtime, &ne, i, flag);
#endif
...
#else
	      ne = SphP[i].elec;
	      tcool = Um_GetCoolingTime(egyhot, SphP[i].d.Density * a3inv, &ne, i);
#endif
	      ...
	      if(dt > 0)
		{
		  if(P[i].TimeBin)	/* upon start-up, we need to protect against dt==0 */
			...
			gauss2gadget /= All.HubbleParam;
		      ...
		      double mag_seed =
			All.SnSeedField * pow((All.SnSeedRadius / All.SnSeedBubble),
					      2) * pow((All.SnSeedBubble / (All.Time * PPP[i].Hsml)), 3);

		      double mag_norm =
			pow(PPP[i].Hsml,
			    3) * sqrt(0.5 * pow(All.SnSeedSoftening,
						3) * (1.0 + pow(All.SnSeedSoftening, 3)));

		      double n_acc[3];
		      n_acc[0] = P[i].g.GravAccel[0] + SphP[i].a.HydroAccel[0];
		      n_acc[1] = P[i].g.GravAccel[1] + SphP[i].a.HydroAccel[1];
		      n_acc[2] = P[i].g.GravAccel[2] + SphP[i].a.HydroAccel[2];

		      double n_abs = sqrt(n_acc[0] * n_acc[0] + n_acc[1] * n_acc[1] + n_acc[2] * n_acc[2]);

		      double sn_eff =
			sqrt(0.004 * cloudmass * All.UnitMass_in_g / SOLAR_MASS * All.FactorSN * dtime /
			     tsfr);
		      if(sn_eff < 1.0)
			{
			  double sn_prob = get_random_number(P[i].ID + 6);
			  if(sn_prob < sn_eff)
			    sn_eff = 0.0;
			  else
			    sn_eff = 1.0;
