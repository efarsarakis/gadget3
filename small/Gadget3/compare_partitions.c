...
int compare_particles(int ncheck, int count)
{
  ...
  for(n = 0; n < ncheck; n++)
      ...
      if(a1 != a2)
	{
      ...
      /* compare EnergySN and EnergySNCold */
      a1 = PP1[n].P.EnergySN;
      a2 = PP2[n].P.EnergySN;
      da = fabs(a1 - a2);
      if(da > ACC * fabs(a1))
	{
      ...
      /* compare EnergySN and EnergySNCold */
      a1 = PP1[n].P.EnergySNCold;
      a2 = PP2[n].P.EnergySNCold;
      da = fabs(a1 - a2);
      if(da > ACC * fabs(a1))
	{
	  if(count < MAXPRINT)
	    printf("STEP=%d  id1=%d id2=%d  SN_energy1Cold=%g SN_energy2Cold=%g  da/da1=%g\n",
		   All.NumCurrentTiStep, PP1[n].P.ID, PP2[n].P.ID, a1, a2, da / a1);
	  count++;
	}

      /* compare metallicities and reservoirs */
      for(j = 0, a1 = a2 = da = 0, flag = 0; j < 12; j++)
	{
	  a1 += PP1[n].P.Zm[j] * PP1[n].P.Zm[j];
	  a2 += PP2[n].P.Zm[j] * PP2[n].P.Zm[j];

	  if(PP1[n].P.Zm[j] != PP2[n].P.Zm[j])
	    flag += 1;

	  delta[j] = (PP1[n].P.Zm[j] - PP2[n].P.Zm[j]);
	  da += delta[j] * delta[j];
	}
      a1 = sqrt(a1);
      a2 = sqrt(a2);
	...
	{
	  if(count < MAXPRINT)
	    printf("STEP=%d  id1=%d id2=%d  type=%d Zm1=%g Zm2=%g  dpos/pos1=%g\n",
		   All.NumCurrentTiStep, PP1[n].P.ID, PP2[n].P.ID, PP1[n].P.Type, a1, a2, da / a1);
	  count++;
	}
      ...
      for(j = 0, a1 = a2 = da = 0, flag = 0; j < 12; j++)
	{
	  a1 += PP1[n].P.ZmReservoir[j] * PP1[n].P.ZmReservoir[j];
	  a2 += PP2[n].P.ZmReservoir[j] * PP2[n].P.ZmReservoir[j];

	  if(PP1[n].P.ZmReservoir[j] != PP2[n].P.ZmReservoir[j])
	    flag += 1;

	  delta[j] = (PP1[n].P.ZmReservoir[j] - PP2[n].P.ZmReservoir[j]);
	  da += delta[j] * delta[j];
	}
      a1 = sqrt(a1);
      a2 = sqrt(a2);
	...
	{
	  if(count < MAXPRINT)
	    printf("STEP=%d  id1=%d id2=%d  type=%d ZmReservoir1=%g ZmReservoir2=%g  dpos/pos1=%g\n",
		   All.NumCurrentTiStep, PP1[n].P.ID, PP2[n].P.ID, PP1[n].P.Type, a1, a2, da / a1);
	  count++;
	}
#endif
