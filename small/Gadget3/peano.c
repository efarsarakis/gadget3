...
#else
#ifdef OMP_SORT
      omp_qsort(mp, N_gas, sizeof(struct peano_hilbert_data), peano_compare_key);
#else
...
#else
#ifdef OMP_SORT
      omp_qsort(mp + N_gas, NumPart - N_gas, sizeof(struct peano_hilbert_data), peano_compare_key);
#else
