      ...
      */
{
  ...
  if(modeBH == INC_BH)
    return IntegrateIMF(Inf, Sup, IMFv, modeWhat);
  else
    {
      partial_result = 0;
...
double IMFevaluate_byMass_powerlaw(double arg, void *params)
     /*
...
double IMFevaluate_byMass_Larson(double arg, void *params)
     /*
	note: this function does not care whether or not the mass
	arg will end in a BH.
      */
{
  ...
  return IMFp->Atot * pow(1 + arg / IMFp->Params[0], -(IMFp->Slopes.slopes[0]));
}
...
double IMFevaluate_byNum_powerlaw(double arg, void *params)
     /*
	note: this function does not care whether or not the mass
	arg will end in a BH.
      */
...
double  IMFevaluate_byNum_Larson(double arg, void *params)
     /*
	note: this function does not care whether or not the mass
	arg will end in a BH.
      */
{
  ...
  return IMFp->Atot * pow(1 + arg / IMFp->Params[0], -(IMFp->Slopes.slopes[0])) / arg;
}
...
double  IMFevaluate_byEgy_powerlaw(double arg, void *params)
     /*
...
double  IMFevaluate_byEgy_Larson(double arg, void *params)
     /*
	note: this function does not care whether the mass arg
	will end in a BH or in a SnII.
      */
...
int get_IMF_SlopeBin(double m, IMF_Type *IMFp)
{
  int i;
...
#define Npoints 100
  ...
  for(i = 1;
      ...
      i++)
    fprintf(file, "%g %g %g\n",
	    x[i], (*IMFs[num].IMFfunc_byMass)(x[i],  &IMFs[num]), (*IMFs[num].IMFfunc_byNum)(x[i],  &IMFs[num]));
          ...
          for(i = IMFv->NSlopes-2; i>=0; i--)
            {
              ...
              IMFv->A[i] = IMFv->IMFfunc_byNum(mleft, IMFv) /
                IMFv->IMFfunc_byNum(mright, IMFv);
            }
