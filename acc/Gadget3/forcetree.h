...
#ifdef MODGRAV
void *phi_primary_loop(void *p);
void *phi_secondary_loop(void *p);
void   force_update_node_recursive(int no, int sib, int father, int part_of_amr_cell);
#else
...
int ngb_treefind_variable_threads(MyDouble searchcenter[3], MyFloat hsml, int target, int *startnode, int mode,
		          int *exportflag, int *exportnodecount, int *exportindex, int *ngblist, int targettype);
#endif
